...
enum class EOpCode : uchar
{
	...
	LDREL,	// IL(8) LDREL [opsz] An|Rn <- *An±Offset32	Load relative from source address, Offset is signed 32 bit numeric value
	STREL,	// IL(8) STREL [opsz] An|Rn -> *An±Offset32 Store relative to target address, Offset is signed 32 bit numeric value
